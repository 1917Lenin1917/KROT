  ...
  return (
     <div className={"flex justify-center "}>
       <Window w={1108.2} h={752.5} backgroundImg={HomeScreenImg}>
         <button ref={ref} className={"hover_size z-10"} onClick={() => {navigate(-1)}} style={{position: "absolute", top: "100px", left: "93px", width: "60px", height: "60px", backgroundImage: `url(${emergencyExitImg})`}}>
         </button>
         <Outlet/>
       </Window>
     </div>
  ...
  return (
    ...
    <BaseForm>
      {Object.keys(levels).map((lvl, key) => {
        ...
        return <Link key={key} className={"z-10 justify-self-center"} to={`/levels/${p}/${filename}`}>
        <button type={"submit"} className={"z-10 border-black justify-self-center border-4 h-20 text-4xl bg-orange-100 w-[450px]"}>{filename}</button>
      </Link>})}
    </BaseForm>
      }
      </>
  ...
  return (
    <BaseForm>
      <Link className={"z-10 justify-self-center"} to={"/levels/1/"}>
        <button type={"submit"} className={"z-10 border-black justify-self-center border-4 h-20 text-4xl bg-orange-100 w-[450px]"}>Глава I</button>
      </Link>
      <Link className={"z-10 justify-self-center"} to={"/levels/2/"}>
        <button type={"submit"} className={"z-10 border-black justify-self-center border-4 h-20 text-4xl bg-orange-100 w-[450px]"}>Глава II</button>
      </Link>
      <Link className={"z-10 justify-self-center"} to={"/levels/3/"}>
        <button type={"submit"} className={"z-10 border-black justify-self-center border-4 h-20 text-4xl bg-orange-100 w-[450px]"}>Глава III</button>
      </Link>
    </BaseForm>
  )
}