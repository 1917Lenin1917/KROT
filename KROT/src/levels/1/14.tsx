...
export default function Level() {
  ...
  useEffect(() => {
    ...
    axios.post(`${BACKEND_IP}/api/check_level_answer/14`, {
    ...
    <>
      <div className={"grid w-full h-full"} style={{
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-xl"}
             style={{gridArea: "9 / 5 / span 3 / span 36"}}>Результат функции преобразования S-блоков
        </div>
        ...
        {isCorrect1  && <button onClick={() => {
          axios.post(`${BACKEND_IP}/api/complete_level/14`, {token: localStorage.getItem('token')}).then(() => {
            navigate('/levels/1/15')
          })
        }} className={"hover_size border-4 text-3xl border-black bg-orange-200 hover:border-green-700"}
