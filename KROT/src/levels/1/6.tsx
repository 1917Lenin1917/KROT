...
export default function Level() {
  ...
  useEffect(() => {
    ...
    axios.post(`${BACKEND_IP}/api/check_level_answer/6`, {
    ...
    <>
      <div className={"grid w-full h-full"} style={{
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "16 / 5 / span 3 / span 36"}}>Что с ним случилось?
        </div>
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "9 / 5 / span 3 / span 36"}}>О каком агенте идёт речь? (Фамилия)
        </div>
        <input className={"p-1 border-black border-4 bg-orange-100"} placeholder={"ВВЕСТИ ОТВЕТ"}
        ...
        {isCorrect1 && isCorrect1 && <button onClick={() => {
          axios.post(`${BACKEND_IP}/api/complete_level/6`, {token: localStorage.getitem('token')}).then(() => {
            navigate('/dialog3')
          })
        ...
        <Link to={"/mail/mail/4"} className={"hover_size"} onClick={() => {
        }}
