...
export default function Level() {
  const hints = [
    'Это проще чем кажется! Какой узор вам напоминает рисунок гор?'
  ]
  ...
  useEffect(() => {
    ...
    axios.post(`${BACKEND_IP}/api/check_level_answer/8`, {
    ...
    <>
      <div className={"grid w-full h-full"} style={{
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "16 / 5 / span 3 / span 36"}}>О каком агенте речь?
        </div>
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "9 / 5 / span 3 / span 36"}}>Что с агентом?
        </div>
        <input className={"p-1 border-black border-4 bg-orange-100"} placeholder={"ВВЕСТИ ОТВЕТ"}
        ...
        {isCorrect1 && isCorrect2 && <button onClick={() => {
          axios.post(`${BACKEND_IP}/api/complete_level/8`, {token: localStorage.getItem('token')}).then(() => {
            navigate('/dialog5')
          })
