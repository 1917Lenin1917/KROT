...
export default function Level() {
  const hints = [
    'Не забудьте добавить 0 биты, где это необходимо',
    'Последние две буквы зашифровывать не нужно. (ИЙ)'
  ]
  ...
  useEffect(() => {
    ...
    axios.post(`${BACKEND_IP}/api/check_level_answer/10`, {
    ...
    <>
      <div className={"grid w-full h-full"} style={{
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "16 / 5 / span 3 / span 36"}}>Введите код символов ключа (16 система)
        </div>
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "9 / 5 / span 3 / span 36"}}>Введите код символов фамилии (16 система)
        </div>
        <input className={"p-1 border-black border-4 bg-orange-100"} placeholder={"ВВЕСТИ ОТВЕТ"}
        ...
        {isCorrect1 && isCorrect2 && <button onClick={() => {
          axios.post(`${BACKEND_IP}/api/complete_level/10`, {token: localStorage.getItem('token')}).then(() => {
            navigate('/levels/1/11')
          })
