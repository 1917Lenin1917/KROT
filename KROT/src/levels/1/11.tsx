...
export default function Level() {
  const hints = [
    'В последние две ячейки ответа напишите полученные блоки для шифрования в 16-ой системе',
  ]
  ...
  useEffect(() => {
    ...
    axios.post(`${BACKEND_IP}/api/check_level_answer/11`, {
    ...
    <>
      <div className={"grid w-full h-full"} style={{
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "16 / 5 / span 3 / span 36"}}>Введите L<sub>0</sub> и R<sub>0</sub>
        </div>
        <input className={"p-1 border-black border-4 bg-orange-100"} placeholder={"L = "}
               style={{gridArea: "20 / 5 / span 2 / span 16"}} onChange={(e) => {
          setAnswer2(e.currentTarget.value)
        }}>
        </input>
        <input className={"p-1 border-black border-4 bg-orange-100"} placeholder={"R = "}
               style={{gridArea: "20 / 22 / span 2 / span 16"}} onChange={(e) => {
          setAnswer3(e.currentTarget.value)
        }}>
        </input>
        <button
          className={`${a2Debounce.length === 0 || a3Debounce.length === 0 ? "border-black" : isCorrect2 && isCorrect3 ? "border-green-700" : "border-red-800"} text-[30px] border-4 bg-orange-300`}
          style={{gridArea: "20 / 37 / span 2 / span 4"}}>

        </button>

        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "9 / 5 / span 3 / span 36"}}>Введите переставленные биты, но в 16-ой системе
        </div>
        ...
        {isCorrect1 && isCorrect2 && isCorrect3 && <button onClick={() => {
          axios.post(`${BACKEND_IP}/api/complete_level/11`, {token: localStorage.getItem('token')}).then(() => {
            navigate('/levels/1/12')
          })
        }} className={"hover_size border-4 text-3xl border-black bg-orange-200 hover:border-green-700"}
