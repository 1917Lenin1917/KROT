...
import Чубзик from "@assets/Чубзик3.png"
...
export default function Level() {
  ...
  useEffect(() => {
    ...
    axios.post(`${BACKEND_IP}/api/check_level_answer/9`, {
    ...
    <>
      <div className={"grid w-full h-full"} style={{
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "16 / 5 / span 3 / span 36"}}>Каким шифром будете шифровать?
        </div>
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "9 / 5 / span 3 / span 36"}}>Кто предатель? (ФАМИЛИЯ)
        </div>
        <input className={"p-1 border-black border-4 bg-orange-100"} placeholder={"ВВЕСТИ ОТВЕТ"}
        ...
        <Link to={"/hint"} state={{ background: location, hints: hints, chel: Чубзик }} className={"hover_size border-4 text-3xl border-black bg-orange-200 hover:border-green-700"}
                style={{gridArea: "32 / 5 / span 3 / span 10"}}>ПОДСКАЗКА
        </Link>

        {isCorrect1 && isCorrect2 && <button onClick={() => {
          axios.post(`${BACKEND_IP}/api/complete_level/9`, {token: localStorage.getItem('token')}).then(() => {
            navigate('/levels/1/10')
          })
