...
export default function Level() {
  ...
  useEffect(() => {
    ...
    axios.post(`${BACKEND_IP}/api/check_level_answer/13`, {
    ...
    <>
      <div className={"grid w-full h-full"} style={{
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-xl"}
             style={{gridArea: "18 / 5 / span 3 / span 36"}}>Сгруппируйте -S-блоки. Сколько блоков должно получиться?
        </div>
        <input className={"p-1 border-black border-4 bg-orange-100"} placeholder={"ВВЕДИТЕ ОТВЕТ"}
               style={{gridArea: "21 / 5 / span 2 / span 32"}} onChange={(e) => {
        ...
        <button
          ...
          style={{gridArea: "21 / 37 / span 2 / span 4"}}>

        </button>

        <div className={"p-1 border-black border-4 bg-orange-100 text-xl"}
             style={{gridArea: "9 / 5 / span 4 / span 36"}}>Сложите по модулю 2 ключ (после конечной перестановки) с блоком текста R
        </div>
        ...
        {isCorrect1 && isCorrect2 && <button onClick={() => {
          axios.post(`${BACKEND_IP}/api/complete_level/13`, {token: localStorage.getItem('token')}).then(() => {
            navigate('/levels/1/14')
          })
        }} className={"hover_size border-4 text-3xl border-black bg-orange-200 hover:border-green-700"}
