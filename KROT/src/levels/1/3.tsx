...
export default function Level() {
  ...
  useEffect(() => {
    ...
    axios.post(`${BACKEND_IP}/api/check_level_answer/3`, {
    ...
    <>
      <div className={"grid w-full h-full"} style={{
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "24 / 5 / span 3 / span 36"}}>ФИО 3 агента в списке
        </div>
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "16 / 5 / span 3 / span 36"}}>ФИО 2 агента в списке
        </div>
        <input className={"p-1 border-black border-4 bg-orange-100"} placeholder={"ВВЕСТИ ОТВЕТ"}
        ...
        <div className={"p-1 border-black border-4 bg-orange-100 text-2xl"}
             style={{gridArea: "9 / 5 / span 3 / span 36"}}>ФИО 1 агента в списке
        </div>
        <input className={"p-1 border-black border-4 bg-orange-100"} placeholder={"ВВЕСТИ ОТВЕТ"}
        ...
        {isCorrect1 && isCorrect2 && isCorrect3 && <button onClick={() => {
          axios.post(`${BACKEND_IP}/api/complete_level/3`, {token: localStorage.getItem('token')}).then(() => {
            navigate('/levels/1/4')
          })
        }} className={"hover_size border-4 text-3xl border-black bg-orange-200 hover:border-green-700"}
                                             style={{gridArea: "32 / 31 / span 3 / span 10"}}>ДАЛЕЕ
        </button>}
